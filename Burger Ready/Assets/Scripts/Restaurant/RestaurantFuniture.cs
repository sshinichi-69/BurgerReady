...
namespace Restaurant.Furniture
{
    public class RestaurantFurniture : MonoBehaviour
    {
        ...
        private const int restaurantMinZ = -15;
        private const int restaurantMaxZ = 20;
        ...
        {
            float k = 2.5f;
            int nMinHalfRowFloor = Mathf.RoundToInt(restaurantMinZ / k);
            int nMaxHalfRowFloor = Mathf.RoundToInt(restaurantMaxZ / k);
            int nHalfColFloor = Mathf.RoundToInt(restaurantMaxX / k);
            for (int i = nMinHalfRowFloor; i <= nMaxHalfRowFloor; i++)
            {
        ...
        void GenerateWall()
        {
            ...
            {
                GameObject wallPiece = Instantiate(wallPrefab, new Vector3(j * k, 0, nMaxHalfRowFloor * k), Quaternion.identity);
                wallPiece.transform.parent = wall.transform;
            }
            // right
            for (int i = nMinHalfRowFloor; i <= nMaxHalfRowFloor - 2; i++)
            {
